# Water_Validation_App_FINAL.py
# Streamlit app for automated Water Quality Data Validation (CRP/TST-style)
# FINAL version for your exact headers + DSR-ready + E.coli-ready + exclusion report.
...
# ---------------------------------------------------------------------
# 1. CONFIG – COLUMN NAMES (your exact headers + minor variants)
# ---------------------------------------------------------------------
COLUMN_MAP = {
...
def find_col_fuzzy(df, required_keywords):
    """Fuzzy match by required keywords."""
    if df is None or df.empty:
        return None
    for c in df.columns:
        s = _norm_name(c)
...
def categorize_columns(df):
    ...
    ecoli_keys = [
        "ecoli_avg","ecoli_cfu1","ecoli_cfu2","ecoli_colonies1","ecoli_colonies2",
    ...
    def add_cols(keys, target_list):
        for key in keys:
            ...
            if c is None:
                # Fuzzy backups for critical fields
                if key == "ecoli_avg":
...
def filter_dsr_ready(df, category_cols):
    """
    DSR filtering:
      - Non-E. coli parameters: >=10 valid events per parameter per site
      - E. coli: ONLY check 'E. Coli Average' (Jenna rule)
      - Watershed must have >=3 sites (if watershed exists)
    Returns: dsr_ready_df, exclusion_report
    """
    df = df.copy()
    site_col = find_col(df, COLUMN_MAP["site"])
    watershed_col = find_col(df, COLUMN_MAP["watershed"])
...
def filter_ecoli_ready(df, min_events=10):
    """
    E. coli-only ready dataset:
      - keep ONLY sites with >= min_events valid E. Coli Average values
      - keep ONLY rows where E. Coli Average is not NaN
    Returns: ecoli_ready_df, good_site_list
    """
    df = df.copy()
    site_col = find_col(df, COLUMN_MAP["site"])
    ecoli_avg_col = find_col(df, COLUMN_MAP["ecoli_avg"]) or find_col_fuzzy(df, ["coli","average"])

    if (not site_col) or (not ecoli_avg_col):
        return df, []

    ecoli_counts = (
        df.groupby(site_col)[ecoli_avg_col]
          .apply(lambda x: x.notna().sum())
          .reset_index(name="n_ecoli")
    )
    good_sites = ecoli_counts[ecoli_counts["n_ecoli"] >= min_events][site_col].tolist()

    ecoli_ready_df = df[df[site_col].isin(good_sites) & df[ecoli_avg_col].notna()].copy()
    return ecoli_ready_df.reset_index(drop=True), good_sites
...
with tabs[1]:
    ...
    if not has_data:
        ...
        if site_col and desc_col:
            ...
            st.info("No explicit description column detected (skipping check).")
...
with tabs[7]:
    ...
    if not has_data:
        ...
        if apply_dsr_filter:
            ...
            st.markdown("### Exclusion Report (why removed)")
            st.dataframe(exclusion_report)

            # E. coli-only ready dataset
            ecoli_ready_df, ecoli_good_sites = filter_ecoli_ready(clean_df, min_events=10)
            st.markdown("### E. coli-ready sites (≥10 E. Coli Average values)")
            st.write(ecoli_good_sites)
            st.markdown("### Preview E. coli-ready dataset")
            st.dataframe(ecoli_ready_df.head(30))

        else:
            dsr_ready_df = clean_df.copy()
            exclusion_report = pd.DataFrame()
            ecoli_ready_df = pd.DataFrame()
            ecoli_good_sites = []
        ...
        # E. coli-ready CSV
        if isinstance(ecoli_ready_df, pd.DataFrame) and not ecoli_ready_df.empty:
            buf_ecoli = io.BytesIO()
            ecoli_ready_df.to_csv(buf_ecoli, index=False)
            st.download_button(
                label="Download E. coli-ready CSV (≥10 E. coli events only)",
                data=buf_ecoli.getvalue(),
                file_name="cleaned_data_ECOLI_ready.csv",
                mime="text/csv",
                key="download_ecoli_ready"
            )

        # Exclusion report CSV
        if isinstance(exclusion_report, pd.DataFrame) and not exclusion_report.empty:
            buf_excl = io.BytesIO()
...
- Ensure consistent sampling time-of-day (QC flag only).
- DSR-ready uses ≥10 valid values per parameter per site.
- Watershed must have ≥3 sites (if watershed exists).

### CORE
- Sample depth must be 0.3 m or half of total depth.
- Total depth 0 only allowed for dry/no water.
- DO duplicates within 0.5 mg/L.
- Secchi ≤ total depth; 2 sig figs.
- Transparency tube ≤1.2 m.
- TDS = Conductivity × 0.65.
- Physical range checks for pH, temp, etc.

### E. COLI
- 0 values → NaN.
- Colonies ≥200 → NaN.
- Temp 30–36 °C QC flag.
- Hold 28–31 h QC flag.
- Field blank QC flag.
- DSR 10-event rule uses ONLY E. Coli Average.
- Separate E. coli-ready export keeps only sites with ≥10 E. Coli Average values.

### ADVANCED
- Turbidity negatives removed.
- Discharge rounding rules.

### RIPARIAN
- Basic completion/image QC flags.
        """
    )
